...
/**
 * Hardcoded admin credentials for the Admin Panel login gate.
 * These are intentionally client-side-only and separate from user accounts.
...
export function authenticateUser(email, password) {
  ...
  // Allows admin@gmail.com / admin123 to always work, even before registration.
  // Auto-seeds the admin account in localStorage on first use (idempotent).
  if (normalizedEmail === ADMIN_EMAIL.toLowerCase() && password === ADMIN_PASSWORD) {
    const users = getRegisteredUsers();
    let adminUser = users.find((u) => u.email === normalizedEmail);
    if (!adminUser) {
      // First-time seed: create the admin user record automatically
